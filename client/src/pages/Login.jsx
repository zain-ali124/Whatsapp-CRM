...
export default function Login() {
  ...
  return (
    <div className="h-screen overflow-hidden flex bg-background-light dark:bg-background-dark font-display">
      ...
      <motion.div
        ...
        className="hidden xl:flex lg:w-1/2 bg-primary/8 dark:bg-primary/5 items-center justify-center p-12 relative overflow-hidden"
      >
        {/* Grid bg */}
      ...
      {/* Right: Form - Full width on mobile/tablet */}
      <motion.div
        initial={{ opacity: 0, x: 40 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.6, ease: 'easeOut' }}
        className="w-full xl:w-1/2 flex flex-col justify-center items-center p-4 sm:p-8 bg-white dark:bg-background-dark overflow-hidden"
      >
        <div className="w-full max-w-md">
          {/* Brand */}
          <div className="flex items-center gap-2.5 mb-6">
            <div className="size-9 bg-primary rounded-xl flex items-center justify-center text-white shadow-[0_4px_12px_rgba(16,183,127,0.3)]">
              <span className="material-symbols-outlined text-[18px]">forum</span>
            </div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">WhatsApp CRM</h2>
          </div>
          ...
          <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl mb-5">
            {[
              ...
              <button key={value} type="button" onClick={() => setMode(value)}
                className={`flex-1 flex items-center justify-center gap-2 py-1.5 rounded-lg text-xs font-bold transition-all ${mode === value
          ...
          {mode === 'agent' && (
            <div className="flex items-start gap-2 p-3 bg-primary/5 border border-primary/10 rounded-xl mb-4">
              <span className="material-symbols-outlined text-[18px] text-primary shrink-0 mt-0.5">info</span>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">Agent Login: Use the shared credentials provided by your manager.</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5 px-0.5">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="input-field pr-12 h-11 text-sm"
                />
                <button
                  type="button"
            ...
            <div className="flex items-center gap-2 px-0.5">
              <input type="checkbox" id="remember" className="size-4 text-primary border-slate-300 rounded focus:ring-primary/20" />
              <label htmlFor="remember" className="text-xs font-medium text-slate-600 dark:text-slate-400 cursor-pointer">Remember me on this device</label>
            </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.99 }}
              className="btn-primary w-full h-11 text-sm font-bold shadow-lg shadow-primary/10 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
          ...
          <p className="mt-5 text-center text-xs text-slate-600 dark:text-slate-400 font-medium">
            No account?{' '}
            <Link to="/register" className="font-bold text-primary hover:underline">Start free trial</Link>
          </p>

          <footer className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800 text-center text-[10px] text-slate-400">
            © 2026 WhatsApp CRM Platform. Enterprise Edition.
          </footer>
        </div>
      </motion.div>
    </div>
